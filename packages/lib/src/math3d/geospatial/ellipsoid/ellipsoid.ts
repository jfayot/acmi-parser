...
const scratchLocalToFixedMatrix = new Matrix4();
const scratchFixedToLocalMatrix = new Matrix3();
const scratchQuaternionMatrix = new Matrix3();
const scratchVelocity = new Vector3();
const scratchRight = new Vector3();
const scratchUp = new Vector3();
const scratchHprQuaternion = new Quaternion();
const scratchHprMatrix = new Matrix4();
const scratchHprRotation = new Matrix3();
...
export class Ellipsoid {
  ...
  static readonly WGS84: Ellipsoid = new Ellipsoid(
    WGS84_RADIUS_X,
    WGS84_RADIUS_Y,
    WGS84_RADIUS_Z
  );
  ...
  readonly squaredXOverSquaredZ: number = 0;
  ...
  cartesianToCartographic(
    cartesian: Readonly<NumericArray>,
    result: Vector3
  ): Vector3;
  cartesianToCartographic(
    cartesian: Readonly<NumericArray>,
  ...
  cartesianToCartographic(
    ...
    const point = this.scaleToGeodeticSurface(
      scratchCartesian,
      scratchPosition
    );
  ...
  eastNorthUpToFixedFrame<T extends NumericArray>(
   ...
   */
  localFrameToFixedFrame<T extends NumericArray>(
  ...
  geocentricSurfaceNormal<NumArray>(
    cartesian: number[],
    result: NumArray
  ): NumArray;
  ...
  geodeticSurfaceNormal<NumArrayT>(
    cartesian: number[],
    result: NumArrayT
  ): NumArrayT;
  ...
  getEulerFromPositionQuaternion(
      ...
      scratchQuaternion
        .fromMatrix3(
          this.localFrameToFixedFrame(
            ...
            "up",
            position,
            scratchLocalToFixedMatrix
          )
            .invert()
            .getRotationMatrix3(scratchFixedToLocalMatrix)
            .multiplyRight(scratchQuaternionMatrix.fromQuaternion(quaternion))
        )
        .normalize()
    );
    return result;
  ...
  getRotationMatrixFromPositionVelocity(
    position: Readonly<NumericArray>,
    velocity: Readonly<NumericArray>,
    result = new Matrix3()
  ): Matrix3 {
    const cartesian = scratchCartesian.fromArray(position);
    const normal = this.geodeticSurfaceNormal(cartesian, scratchNormal);

    const v = scratchVelocity.fromArray(velocity);
    let right = Vector3.cross(velocity, normal, scratchRight);

    if (right.equals(Vector3.ZERO)) {
      right.fromArray(Vector3.X_AXIS);
    }

    const up = Vector3.cross(right, velocity, scratchUp).normalize();
    right = Vector3.cross(velocity, up, scratchRight).negate().normalize();
  ...
  getQuaternionFromEuler(
    ...
    return result.fromMatrix3(
      this.localFrameToFixedFrame(
        ...
        .multiplyRight(
          scratchHprMatrix.fromTranslationQuaternion(
            Vector3.ZERO,
            scratchHprQuaternion.fromEuler(e)
          )
        )
        .getRotationMatrix3(scratchHprRotation)
    );
  }
}
